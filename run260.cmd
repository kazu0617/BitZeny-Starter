...
if /i "%input%" == "1" ( set quiet=-q )
if %cthread%==true (
  call cmd /c start "BitZeny-Miner ( %quiet% CustomThread:%cthread% | pool:%server% / %stratum% / %address% | Affinity:%affinity% )" /high /affinity %affinity% minerd260.exe %quiet% -t %cthreadnumber% -a yescrypt -o %stratum% -u %address%.%worker% --cpu-affinity %affinity%
) else (
  call cmd /c start "BitZeny-Miner ( %quiet% CustomThread:%cthread% Num:%cthreadnumber% | pool:%server% / %stratum% / %address% | Affinity:%affinity% )" /high minerd260.exe %quiet% -a yescrypt -o %stratum% -u %address%.%worker% --cpu-affinity %affinity%
)
echo 実行されました。数秒後に設定画面に戻ります
set input=3
timeout 5 > nul
